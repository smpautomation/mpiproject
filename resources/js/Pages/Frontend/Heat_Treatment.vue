<template>
    <Frontend>
        <div
            class="flex flex-row items-start gap-10 p-5 text-green-600 bg-black"
        >
            <div
                    ...
                    <Modal :show="msgPatternNotAllowed" @close="msgPatternNotAllowed = false">
                        ...
                        <div class="relative flex flex-col items-start bg-white p-6 rounded-xl shadow-2xl max-w-[95vw] max-h-[90vh] overflow-auto pr-12">
                            ...
                            <div class="flex items-center gap-3 mb-4 w-full">
                                ...
                                <div class="flex items-center justify-center p-2 text-white rounded-lg shadow-sm bg-gradient-to-r from-amber-500 to-orange-500 shrink-0">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                            ...
                            <!-- Action Button Controls Footer Group -->
                            <div class="mt-6 pt-4 border-t border-slate-100 flex flex-col sm:flex-row gap-2.5 w-full">
                                <!-- Redirect Button -->
                                <button
                                    type="button"
                                    @click="router.visit('ht_graph_patterns')"
                                    class="px-4 py-2 text-xs font-bold text-white transition-all duration-150 rounded-xl shadow-sm bg-gradient-to-r from-teal-600 to-cyan-500 hover:from-teal-700 hover:to-cyan-600 focus:outline-none whitespace-nowrap active:scale-[0.98]"
                                >
                                ...
                                <button
                                    type="button"
                                    @click="msgPatternNotAllowed = false"
                                    class="px-4 py-2 text-xs font-bold text-slate-700 transition-all duration-150 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 hover:text-slate-900 focus:outline-none whitespace-nowrap active:scale-[0.98]"
                                >
                                    Okay, I understand
                                </button>
                            </div>
...
const msgPatternNotAllowed = ref(false);
const msgPatternNotExists = ref(false);
//Dev Controls ----------------- Allow Commands
...
const getMassProdData = async () => {
    ...
    try {
        ...
        const hasExisting = !!(
            massProdData.batch_cycle_no && massProdData.machine_no
        );
...
const totalWt = computed(() => {
    ...
    const excessTotal =
        mpcs.moreThanTenBoxes || mpcs.breakLotMode
            ? visibleExcessBoxes.value.reduce(
                  (sum, box) =>
                      sum + Number(weightValuesExcess.value[box] || 0),
                  0,
              )
            : 0;
...
const getGrandTotalWeightData = async () => {
    try {
        const response = await axios.post(
            "/api/mass-production/grand-total-weight",
            {
                mass_prod: mpcs.selectedMassProd,
                furnace: mpcs.selectedFurnace,
            },
...
const getTotalBoxes = async () => {
    try {
        const response = await axios.post(
            "/api/initial-control-sheets/lot-total-boxes",
            {
...
const getGraphPatterns = async () => {
    try {
        const response = await axios.get(
            "/api/htgraph-patterns/related-lists",
            {
                params: {
...
const fetchExistingLayers = async () => {
    ...
    try {
        ...
        if (mpcs.selectedLayer) {
            ...
            isExisting_2ndGBDP.value = existingLayers_2ndGBDP.value.includes(
                mpcs.selectedLayer,
            );

            isExistingExcess.value = existingExcessLayers.value.includes(
                mpcs.selectedLayer,
            );
        }
        ...
        if (isExisting_2ndGBDP.value) {
            toast.warning(
                "Selected layer already contains existing 1st and 2nd gbdp data.",
            );
        }
...
const changeData = () => {
    ...
    msgPatternNotExists.value = false;
...
/**
 * Cross-references current graph_patterns with the allowed model configurations
            ...
            if (Array.isArray(allowedPatterns)) {
                if (hti && hti.patternNo !== undefined && hti.patternNo !== null) {
                    ...
                    if (!allowedPatterns.includes(currentPatternNum)) {
                        ...
                        msgPatternNotAllowed.value = false;
                    }

                } else {
                    // Fallback: If no pattern number has been loaded yet, keep the warning hidden
                    msgPatternNotAllowed.value = false;
                }
            }
        } else {
            console.log("No explicit model rules found to filter patterns. Bypass active.");
        }
...
const validateSelectedModel = async () => {
    try {
        ...
        if (firstResponse.data && firstResponse.data.rule_existing) {
            ...
            msgModelNotAllowed.value = false;
            msgPatternNotExists.value = true;
            await fetchAllLotDataBoxDetails();
        }
...
const fetchAllLotDataBoxDetails = async () => {
    ...
    try {
        ...
        if (!sheet) {
            toast.warning(
                "The selected Model does not exist in the selected Lot No",
            );
            return;
        }
        ...
        if (total === 10) {
            ...
            if (mpcs.selectedLayer === "9.5" && mpcs.nineHalfSet) {
                ...
                visibleBoxes.value = visibleBoxes.value.filter((box) =>
                    allowedBoxes.includes(box),
                );

                // Remove values for boxes not in 9.5 set
                Object.keys(qtyValues.value).forEach((box) => {
                    if (!visibleBoxes.value.includes(box))
                        qtyValues.value[box] = "";
                });
        ...
        if (total < 10) {
            ...
            if (hasExcess) {
                ...
                mpcs.selectedBoxEndList = visibleBoxes.value.at(-1);
                mpcs.selectedExcessBoxEndList =
                    visibleExcessBoxes.value.at(-1) || null;
        ...
        toast.error(
            "Failed to fetch initial process data via lot number and model",
        );
        console.log(error);
    }
};
...
const loadNormalBoxData = (layerData) => {
    ...
    const getRow = (title) =>
        layerData.find((r) => r.rowTitle === title)?.data || {};
    ...
    let allBoxes = Object.keys(boxNoRow).filter(
    ...
    mpcs.magnetPreparedBy = magnetRow[firstBox] || "";
    mpcs.boxPreparedBy = boxPrepRow[firstBox] || "";
    mpcs.selectedBoxEndList =
        visibleBoxes.value[visibleBoxes.value.length - 1] || firstBox;
...
const finalize = async () => {
    if (!bypassValidation.value) {
        ...
        await validatePatternRules();
        if(msgPatternNotAllowed.value){
            return;
        }
    }
    ...
    if (isLayerNinePointFive.value && mpcs.nineHalfSet) {
        if (mpcs.nineHalfSet === "SET1") {
            boxesToValidate = visibleBoxes.value.filter((b) =>
                ["A", "C", "E", "G", "J"].includes(b),
            );
        } else if (mpcs.nineHalfSet === "SET2") {
            boxesToValidate = visibleBoxes.value.filter((b) =>
                ["B", "D", "F", "H", "K"].includes(b),
            );
        }
    }
...
const saveToDatabase = async () => {
    ...
    // Determine layer key
    const layerKey =
        mpcs.selectedLayer === "9.5"
            ? "layer_9_5"
            : `layer_${mpcs.selectedLayer}`;
    ...
    try {
        ...
        if (
            (mpcs.moreThanTenBoxes || mpcs.breakLotMode) &&
            visibleExcessBoxes.value.length
        ) {
            const excessLayerPayload = formatLayerDataForDatabase(
            ...
            const saveExcess = await axios.post(
...
const formatLayerDataForDatabase = (boxes, isExcess = false) => {
    ...
    const totalQty = boxes.reduce(
        (acc, box) => acc + Number(qtyMap[box] || 0),
        0,
    );
    ...
    return [
        {
            ...
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.selectedModel || ""]),
            ),
        },
        {
            rowTitle: "COATING M/C No.:",
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.coatingMCNo || ""]),
            ),
        },
        {
            rowTitle: "LT. No.:",
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.lotNo || ""]),
            ),
        },
        {
            rowTitle: "QTY (PCS):",
            data: Object.fromEntries(
                boxes.map((box) => [box, qtyMap[box] || 0]),
            ),
        },
        {
            rowTitle: "HT (PCS):",
            data: Object.fromEntries(
                boxes.map((box) => [box, htMap[box] || 0]),
            ),
        },
        {
            rowTitle: "LT (PCS):",
            data: Object.fromEntries(
                boxes.map((box) => [box, ltMap[box] || 0]),
            ),
        },
        {
            rowTitle: "COATING:",
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.coating || 0]),
            ),
        },
        {
            rowTitle: "WT (KG):",
            data: Object.fromEntries(
                boxes.map((box) => [box, weightMap[box] || 0]),
            ),
        },
        {
            rowTitle: "BOX No.:",
            data: Object.fromEntries(
                boxes.map((box) => [box, boxNoMap[box] || ""]),
            ),
        },
        {
            rowTitle: "Magnet prepared by:",
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.magnetPreparedBy || ""]),
            ),
        },
        {
            rowTitle: "Box prepared by:",
            data: Object.fromEntries(
                boxes.map((box) => [box, mpcs.boxPreparedBy || ""]),
            ),
        },
        {
        ...
        {
            rowTitle: "TOTAL QTY",
            data: Object.fromEntries(boxes.map((box) => [box, totalQty])),
        },
    ];
};
...
const overwriteDatabase = async () => {
    if (!hti.cyclePattern || !hti.currentPattern) {
        toast.warning("Cycle Pattern and Current Pattern input are required");
        return;
    }
    ...
    try {
        ...
        if (!heatTreatmentInformationDetected.value) {
            await userManageLogging(
                "overwritten " +
                    mpcs.selectedMassProd +
                    " Heat Treatment Info successfully | Control Sheet Layer: " +
                    mpcs.selectedLayer,
            );
        } else {
            await userManageLogging(
...
const updateFormatType = async () => {
    ...
    const layerKey =
        mpcs.selectedLayer === "9.5"
            ...
            : `layer_${mpcs.selectedLayer}_format_type`;
...
const uploadGraphs = async () => {
    ...
    try {
        ...
        toast.error(
            "Failed to upload graphs. Please try again. Furnace: ",
            mpcs.selectedFurnace,
            "Mass prod: ",
            mpcs.selectedMassProd,
        );
...
const deleteLayerData = async () => {
    try {
        ...
        if (response.data.success) {
            ...
            await fetchExistingLayers();
            await userManageLogging(
                `has removed Layer ${mpcs.selectedLayer} data from ${mpcs.selectedMassProd} | ${mpcs.selectedFurnace} successfully.`,
            );
        } else {
...
.finalize-btn {
    ...
    box-shadow:
        ...
        0 4px 6px -2px rgba(34, 197, 94, 0.25);
    transition:
        transform 0.2s ease,
...
.finalize-btn:hover {
    ...
    box-shadow:
        0 15px 25px -5px rgba(22, 163, 74, 0.6),
        0 6px 8px -3px rgba(22, 163, 74, 0.3);
}
.finalize-btn:active {
