<template>
    <Frontend>
        <div class="flex flex-col items-center justify-start min-h-screen px-8 py-12 space-y-8 bg-gradient-to-b from-gray-50 to-gray-100">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-sm font-semibold text-left text-gray-700 uppercase">Furnace No</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                        v-for="furnace in filteredFurnaceData"
                        :key="furnace.furnace_id"
                        class="transition hover:bg-indigo-50"
                        >
                            <td class="px-6 py-4 text-sm text-gray-700">
                                {{ new Date(furnace.created_at).toLocaleDateString() }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <button
                                    @click="openDeleteConfirm(furnace.furnace_id)"
            ...
            <!-- Empty State -->
            <div
                v-if="filteredFurnaceData.length === 0"
                class="flex flex-col items-center justify-center w-full max-w-5xl p-12 mt-6 text-center bg-white rounded-lg shadow-md"
            >
                <p class="text-gray-500">No furnace records found.</p>
            </div>

            <Modal :show="showModalCreate" @close="showModalCreate = false">
            <div
                class="relative flex flex-col items-center justify-center w-full max-w-2xl p-8 mx-auto bg-white shadow-2xl rounded-2xl animate-fadeIn"
            >
                <!-- Exit Button -->
                <button
                ...
                <h2 class="text-2xl font-bold text-gray-800">➕ New Furnace</h2>
                <p class="text-sm text-gray-500">Fill in the details to create a new furnace record.</p>
                </div>
                ...
                <div class="flex flex-col w-full">
                    ...
                    type="text"
                    v-model="furnaceNo"
                    @input="furnaceNo = furnaceNo.toUpperCase()"
                    placeholder="ex. K-40"
...
const filteredFurnaceData = computed(() => {
    return furnaceDataLists.value.filter(furnace => {
...
const getFurnaceLists = async () => {
    try {
        ...
        console.error("Failed to fetch furnace data", error);
        toast.error("Failed to get furnace data.");
    }
};
...
const deleteFurnace = async () => {
    ...
    try {
        ...
        // furnaceDataLists.value = furnaceDataLists.value.filter(f => f.furnace_id !== deleteTargetId.value);
    } catch (error) {
        toast.error("Failed to delete furnace.");
    } finally {
        closeDeleteConfirm();
    }
};
