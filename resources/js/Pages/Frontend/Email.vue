<template>
    <Frontend>
        ...
        >
            <div class="pb-6 border-b-2 border-gray-200">
                <div class="flex items-center gap-3">
                    <div
                        class="w-1.5 h-12 bg-gradient-to-b from-teal-500 to-cyan-500 rounded-full"
                    ></div>
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Fill out all fields to send an email
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form @submit.prevent="submitEmail" class="space-y-6">
                ...
                <div>
                    ...
                    <label
                        for="additionalFiles"
                        class="flex flex-col items-center justify-center w-full px-4 py-6 text-center transition-colors border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:border-cyan-400 hover:bg-gray-50"
                    >
                        <svg
                        ...
                        <span class="text-sm text-gray-600"
                            >Click to upload Excel files</span
                        >
                    </label>
                    ...
                    <div
                        v-if="form.additionalFiles.length"
                        class="flex flex-col gap-2 mt-2"
                    >
                        <ul class="text-sm text-gray-700 list-disc list-inside">
                            <li
                                v-for="(file, index) in form.additionalFiles"
                                :key="index"
                            >
                                {{ file.name }}
                            </li>
                        ...
                        <button
                            ...
                            class="self-start px-3 py-1 text-xs font-semibold text-white transition-colors bg-red-500 rounded hover:bg-red-600"
                        >
                            Clear All
                        </button>
                    </div>
            ...
            <div
                v-if="success"
                class="p-4 border-l-4 border-green-500 rounded-r-lg bg-green-50"
            >
                <p
            ...
            >
                <div class="relative w-20 h-20">
                    <div
                        class="absolute inset-0 border-4 border-teal-200 rounded-full animate-spin border-t-teal-500"
                    ></div>
                    <div
                        class="absolute inset-0 flex items-center justify-center"
                    >
                        <svg
                            class="w-10 h-10 text-teal-600 animate-pulse"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                        >
                ...
                <div class="mt-4 space-y-1 text-center">
                    ...
                    <div class="flex items-center justify-center gap-1">
                        <span
                            class="w-2 h-2 bg-teal-500 rounded-full animate-bounce"
        ...
        >
            <!-- Backdrop -->
            <div
                class="absolute inset-0 transition-opacity duration-300 bg-black bg-opacity-40 backdrop-blur-md"
            ></div>

            <!-- Error box -->
            <div
                class="relative w-full max-w-md p-6 mx-4 text-gray-900 transition-transform transform scale-95 bg-white border border-red-200 shadow-2xl rounded-xl animate-fade-in"
            >
                <h3 class="mb-2 text-lg font-semibold text-red-600">Error</h3>
                <p class="text-sm leading-relaxed">{{ serverError }}</p>

                <!-- Close button -->
                <button
                    @click="serverError = null"
                    class="absolute text-lg font-bold text-gray-500 top-3 right-3 hover:text-gray-700"
                    aria-label="Close"
                >
                    ✕
                </button>
...
const submitEmail = async () => {
    ...
    try {
        ...
        await userEmailLogging(
            `has successfully sent an email for ${form.massPro}`,
        );
        ...
        if (error.response && error.response.status === 422) {
            ...
            serverError.value =
                "Failed to send email. Check inputs or network.";
            console.error(error);
        }
    } finally {
        loading.value = false;
